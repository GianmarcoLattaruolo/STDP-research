...
class LIFNeuron:
    ...
    def forward(self, I_inj):
        ...
        self.fmp = (
            self.fmp * (1 - dt / self.tau_m)
            + dt / self.tau_m * self.U_resting
            + dt / self.tau_m * self.R * I_inj
        )
    ...
    def get_records(self):
        if self.dynamic_threshold:
            return {
                "mem": np.array(self.mem_record),
                "FMP": np.array(self.fmp_record),
                "spk": np.array(self.spk_record),
                "I_inj": np.array(self.I_inj_record),
...
class LIF_syn_conductance(LIFNeuron):
    ...
    def forward(self, I_inj_exc, I_inj_inh=0):
        ...
        if spk:
            ...
            if self.dynamic_threshold:
                self.threshold = (
                    self.pars["ratio_thr"] * (self.threshold - self.U_resting)
                    + self.U_resting
                )
        ...
        drift = (
            dt
            / self.tau_m
            * (self.U_resting + self.g_E * self.V_exc + self.g_I * self.V_inh)
        )
    ...
    def plot_records(
        self, title=False, time_in_ms=False, show_fmp=False, show_conductances=True
    ):
...
class PoissonNeuron:
    ...
    def forward(self, I_inj):
        ...
        )  # I should improve this step according to the theory
        rate = rate * (rate > 0)
        ...
        # update the membrane potential
        self.mem = self.mem + (dt / self.tau_m) * (
            self.U_resting - self.mem + I_inj * self.R
        )
        # store the membrane potential
        self.mem_record.append(self.mem)
    ...
    def get_records(self):
        if self.dynamic_threshold:
            return {
                "mem": np.array(self.mem_record),
                "spk": np.array(self.spk_record),
                "rate": np.array(self.rate_record),
                "I_inj": np.array(self.I_inj_record),
                "thr": np.array(self.threshold_record),
            }
        else:
