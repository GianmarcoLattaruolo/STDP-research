...
def weight_initializer(
    pars, N_post, N_pre=None, I=None, type_init=1, my_seed=2024, tensor=False
):
    """
    ARGS:
    ...
    if type_init == 1:
        ...
        W = (
            np.random.rand(N_post, N_pre) * (pars["w_max"] - pars["w_min"])
            + pars["w_min"]
        )
...
def half_growing_rate(dt, num_steps, N_pre, rate_ratio=0.5):
    ...
    second_half = Poisson_generator(
        dt, rate=rate * rate_ratio, N_pre=N_pre, num_steps=num_steps // 2
    )
...
def signals_with_patterns(
    ...
    patterns = Poisson_generator(
        dt, rate=min(rate * 1.25, 0.8), N_pre=N_pre, num_steps=length_pattern
    )

    # start times for the specific of the each neurons pattern in the simulation
    w = num_steps // repeats
    start_times = [
        np.random.randint(t * w, (t + 1) * w - length_pattern) for t in range(repeats)
    ]
