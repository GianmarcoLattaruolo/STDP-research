...
class rate_encoded_mnist(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data_sample = self.rate_encode(
            data_sample, num_steps=self.num_steps, gain=self.gain
        )
    ...
    def rate_encode(self, data_sample, num_steps, gain):
        ...
        data_sample = (
            data_sample * gain
        )  # we multiply the data by a gain to increase the firing rate
        data_sample = (
            data_sample + torch.ones_like(data_sample) * self.min_rate
        )  # we add a minimum rate to ensure that the neurons are not silent
    ...
    def visualize_rate_image(self, idx):
        ...
        data_sample = data_sample.view(
            self.num_steps, 28, 28
    ...
    def get_loader(self):
        if self.train:
            ...
            indices = torch.randperm(
                len(self.dataset), generator=self.my_generator
            ).tolist()
            train_indices = indices[:train_size]
            val_indices = indices[train_size : train_size + val_size]
            train_sampler = torch.utils.data.SubsetRandomSampler(
                train_indices, generator=self.my_generator
            )
            val_sampler = torch.utils.data.SubsetRandomSampler(
                val_indices, generator=self.my_generator
            )
    ...
    def get_subset_train_val_test(self, subset=100):
        ...
        test_sampler = torch.utils.data.SubsetRandomSampler(
            test_indices, generator=self.my_generator
        )
        # create the loaders
