...
class snn_hpo:
    ...
    def __init__(
        ...
        pruner=optuna.pruners.MedianPruner(
            n_startup_trials=5, n_warmup_steps=1, interval_steps=1, n_min_trials=20
        ),
    ):
        ...
        self.fixed_params = {
            ...
            "tau_theta": 1000.0,  # time constant for the dynamic threshold
            ...
            "mu_exponent": 2.0,  # exponent for the dynamic constrain on the weights
        }
    ...
    def STDP_params(self, trial):
        ...
        if STDP_type == "classic":
            A_minus = trial.suggest_float(
                "A_minus", 0.00001, 0.001, log=True
            )  # from prelimanires grid search A_minus is much greater than A_plus
    ...
    def log_to_wandb(self, study_type="accuracy"):  # still in development
        if study_type == "rate":
            ...
            if not hasattr(self, "rate_study"):
                raise ValueError(
                    "Log to wandb is possible only after the optimization of the study"
                )
            # otherwise
    ...
    def f_error(anspnpi, asrpts):
        ...
        """
        error = 0.0
        f_out_interval_error = lambda x: (100 - 25 * x) * (x <= 4) + (
            5 * x - 200
        ) / 3 * (x >= 40)
        ...
        error = (
            f_out_interval_error(anspnpi)
            + correction_factor * spk_std
            + 100 * (spk_mean - 0.05) * (spk_mean > 0.05)
        )
        if error < 0:
            print(
                f"anspnpi = {anspnpi:.2e}, spk_std = {spk_std:.2e}, spk_mean = {spk_mean:.2e}, error = {error:.2e}"
            )
    ...
    def best_range_calculation(self, trial):
        ...
        sort_trials_df = self.study_rate.trials_dataframe().sort_values(
        ...
        if self.STDP_type == "classic":
            ...
            best_ranges["A_minus"] = trial.suggest_float(
                "A_minus", min_A_minus, max_A_minus
            )
            best_ranges["A_plus"] = trial.suggest_float(
                "A_plus", min_A_plus, max_A_plus
            )
            best_ranges["theta_add"] = trial.suggest_float(
                "theta_add", theta_add_min, theta_add_max
            )
            best_ranges["ref_time"] = trial.suggest_int(
                "ref_time", ref_time_min, ref_time_max
            )
            ...
            best_ranges["learning_rate"] = trial.suggest_float(
                "learn_rate", min_learnin_rate, max_learnin_rate
            )
            best_ranges["STDP_offset"] = trial.suggest_float(
                "STDP_offset", min_STDP_offset, max_STDP_offset
            )
    ...
    def optimize_rate(self):
        ...
        if not hasattr(self, "study_rate"):
            raise ValueError(
                "The study for the rate stabilization has not been initialized"
            )
        ...
        def objective(trial):
            ...
            subset_loader = rate_encoded_mnist(
                batch_size,
                num_steps=num_steps,
                gain=gain,
                min_rate=min_rate,
                ...
                30
            )  # 2000 samples in the subset

            # define the model
            pars = mnist_pars(
                weight_initializer=trial.suggest_categorical(
                    "weight_initializer", ["clamp", "shift", "norm_row", "random"]
                ),
                **self.STDP_params(
                    trial
                ),  # this two dictionaries must not share any key
                **self.fixed_params,
            )
            ...
            # set user attributes
            total_anspnpi = np.mean(
                model.anspnpi[-len(subset_loader) :], dtype=np.float64
            )
            first_anspnpi = np.mean(
                model.anspnpi[-len(subset_loader)], dtype=np.float64
            )
        ...
        self.study_rate.optimize(
            ...
            callbacks=[
                MaxTrialsCallback(7000, states=(TrialState.COMPLETE,)),
                self.my_callback(),
            ],
            gc_after_trial=True,
    ...
    def optimize_accuracy(self):
        ...
        if not hasattr(self, "study_accuracy"):
            raise ValueError(
                "The study for the accuracy optimization has not been initialized"
            )
        ...
        if len(self.study_rate.trials) < 2:
            raise ValueError(
                "The study for the rate stabilization has not been completed"
            )
        ...
        def objective_accuracy(trial):
            ...
            mnist_train = rate_encoded_mnist(
                batch_size,
                num_steps=num_steps,
                gain=gain,
                min_rate=min_rate,
            ...
            sub_train, sub_val, sub_test = mnist_train.get_subset_train_val_test(
                subset=25
            )

            # define the model
            pars = mnist_pars(
                assignment_confidence=trial.suggest_float(
                    "ass_conf", 0.00001, 0.01, log=True
                ),
                **best_values,
                **best_ranges,
                **self.fixed_params,
            ...
            for epochs in range(num_epochs):
                ...
                with tqdm(total=len(sub_train), unit="batch", ncols=120) as pbar:
                    ...
                    temp_assignments = assign_neurons_to_classes(
                        model, sub_val, verbose=0
                    )
                    percent_of_good_assignments = np.mean(
                        np.array(temp_assignments.conf_status + 0.0), dtype=np.float64
                    )
                    conf_ass.append(percent_of_good_assignments)

                    # compute the accuracy so far
                    accuracy, anspnpi = classify_test_set(
                        model, sub_test, temp_assignments, verbose=0
                    )
    ...
    def print_statistics(self, type="rate"):
        ...
        if type == "rate":
            print(
                f"Study for the rate stabilization statistics of {self.STDP_type} STDP: "
            )
            ...
            print(f"Best Ranges")
            sort_trials_df = study.trials_dataframe().sort_values(
                by="value", ascending=True
            )
            sort_trials_df = sort_trials_df[sort_trials_df["state"] == "COMPLETE"]
