...
def plot_cur_mem_spk(
    cur_in, cur, mem, fmp, spk, thr_line=False, vline=False, title=False
):
    num_steps = len(cur_in)
    # Generate Plots
    fig, ax = plt.subplots(
        4, figsize=(12, 15), sharex=True, gridspec_kw={"height_ratios": [1, 1, 1, 0.4]}
    )
...
def plot_results_21(time_steps, cur_in, spk_record, weight_history):
    # plot the results
    fig, ax = plt.subplots(
        3, figsize=(12, 10), sharex=True, gridspec_kw={"height_ratios": [1, 0.4, 1]}
    )
...
def plot_raster_weight_distr(cur_in, spk_record, weight_history, N_pre_correlated):
    ...
    fig, ax = plt.subplots(
        4, figsize=(12, 15), gridspec_kw={"height_ratios": [1, 0.2, 1, 1]}
    )
    ...
    cor_weights.plot(ax=ax[2], legend=False, color="tab:red", alpha=0.8, lw=1.0)
    uncor_weights.plot(ax=ax[2], legend=False, color="tab:blue", alpha=0.2, lw=1.0)
    ax[2].set_ylabel("Synaptic Weight")
    ax[2].set_xlabel("Time step")
    ...
    ax[3].hist(
        weight_history[time_step, :N_pre_correlated],
        bins=bins,
        color="tab:red",
        alpha=0.8,
        label="correlated",
...
def plot_results_31(
    ...
    fig, ax = plt.subplots(
        ...
        figsize=(12, 20),
        sharex=True,
        gridspec_kw={"height_ratios": [1, 1, 1, 0.4, 1, 1]},
    )
    ...
    # pre synaptic traces
    ax[1].plot(
        time_steps, pre_trace.detach().numpy(), alpha=0.8, lw=max(100 / num_steps, 0.5)
    )
    ...
    # membrane potential
    ax[2].plot(
        time_steps, mem_rec.detach().numpy(), alpha=0.8, lw=max(100 / num_steps, 0.5)
    )
    ...
    ax[4].plot(
        time_steps, post_trace.detach().numpy(), alpha=0.8, lw=max(100 / num_steps, 0.5)
    )
...
def plot_results_32(
    ...
    # plot the input spikes, membrane potentials, output spikes, pre and post synaptic traces and the weights evolution
    fig, ax = plt.subplots(
        8, figsize=(12, 30), gridspec_kw={"height_ratios": [1, 1, 1, 1, 0.4, 1, 1, 1]}
    )
    time_steps = np.arange(num_steps) * dt
    lw = max(100 / num_steps, 0.5)
    ...
    height = ax[0].bbox.height
    ax[0].scatter(
        *torch.where(cur_in), s=2 * height / N_pre, c="black", marker="|", lw=lw
    )
    ax[0].set_ylim([0 - 0.5, N_pre - 0.5])
    ax[0].set_ylabel("Neuron index")
    ax[0].set_title("Input Spikes")
    ...
    ax[4].scatter(
        *torch.where(spk_rec), s=2 * height / N_post, c="black", marker="|", lw=lw
    )
...
def all_post_weights_evolution(weight_history, N_pre_correlated, block_of_index):
    ...
    lw = max(100 / num_steps, 0.5)
    fig, ax = plt.subplots(
        6, figsize=(12, 5 * N_post), gridspec_kw={"height_ratios": [1, 0.5] * N_post}
    )
    ...
    for post_index in range(N_post):
        ...
        uncor_weights = pd.DataFrame(W[:, ~mask])
        cor_weights.plot(
            ax=ax[ax_number], legend=False, color="tab:red", alpha=0.8, lw=lw
        )
        uncor_weights.plot(
            ax=ax[ax_number], legend=False, color="tab:blue", alpha=0.2, lw=lw
        )
        ...
        ax[ax_number + 1].set_title(
            f"Final Synaptic Weight Distribution of Neuron {post_index}"
        )
