...
def mnist_pars(**kwargs):  # DA SISTEMARE
    ...
    pars["dt"] = 1.0  # (FIXED) simulation time step [ms]
    pars["use_min_spk_number"] = (
        False  # (FIXED) use the minimum number of spikes for a forward pass of a batch
    )
    pars["min_spk_number"] = (
        5  # (FIXED) minimum number of spikes for a forward pass of a batch
    )
    pars["store_records"] = True  # (FIXED) store the records of the simulation
    pars["store_subsampling_factor"] = (
        10  # (FIXED) subsampling factor in time steps for storing the records
    )
    pars["weight_initialization_type"] = (
        "clamp"  # type of weight initialization (still in development)
    )
    pars["assignment_confidence"] = (
        0.01  # < of the difference between the max and the second max for the assignment of the label
    )
    ...
    pars["reset_mechanism"] = (
        "subtract"  # (FIXED) reset mechanism for the neuron potential
    )
    ...
    pars["ref_time"] = 2.0  # refractory time in time steps
    ...
    pars["A_minus"] = (
        0.0002  # magnitude of LTD if STDP_type is 'classic' or 'asymptotic
    )
    ...
    pars["mu_exponent"] = (
        2  # (FIXED) exponent for the dynamic weight constraint if STDP_type is 'offset'
    )
    pars["learning_rate"] = (
        0.0001  # learning rate for the weights if STDP_type is 'offset
    )
...
def default_pars(type_parameters="simple", **kwargs):
    ...
    # typical neuron parameters
    pars["threshold"] = 1 if s else -55.0  # spike threshold [mV] !do not change
    pars["tau_m"] = 10.0  # membrane time constant [ms]
    pars["R"] = (
        1 if s else 0.1
    )  # leak resistance [Ohm] with this resistance input current must be of the order of 100 mV !do not change
    pars["U_init"] = 0.0 if s else -65.0  # initial potential [mV]  !do not change
    pars["U_reset"] = 0.0 if s else -75.0  # reset potential [mV]    !do not change
    pars["U_resting"] = (
        0.0 if s else -75.0
    )  # leak reversal potential [mV]    !do not change  in Neuromatch course this is V_L
    pars["t_ref"] = 0.0 if s else 2.0  # refractory time (ms)

    # neuron parameters for conductance based model
    pars["tau_syn_exc"] = 10.0 if s else 5.0  # synaptic time constant [ms]
    pars["tau_syn_inh"] = 10.0 if s else 5.0  # synaptic time constant [ms]
    pars["U_rev_exc"] = 1.0 if s else 0.0  # excitatory reversal potential [mV]
    pars["U_rev_inh"] = -1.0 if s else -80.0  # inhibitory reversal potential [mV]
    pars["max_g"] = 1.0 if s else 0.024  # maximum synaptic conductance [nS]

    # in the case of dynamic threshold
    pars["tau_thr"] = 20  # threshold time constant [ms]
    pars["ratio_thr"] = (
        1.5 if s else 1.1
    )  # relative increment in the threshold due to a spike
    ...
    # time steps
    pars["dt"] = 1.0 if s else 0.1  # simulation time step [ms]  !do not change
    ...
    pars["tau_syn"] = 5.0  # synaptic time constant [ms] if synaptic decay is used
    pars["dynamic_weight_exponent"] = 0.01  # exponent for the dynamic weight constraint

    # weight parameters
    pars["w_max"] = 1.0 if s else 0.024  # maximum weight
...
def weight_initializer(
    pars, N_post, N_pre=None, I=None, my_seed=2024, type_init=3, tensor=False
):
    """
    ARGS:
    ...
    if N_pre is None and I is not None:
        ...
        if N_pre != np.shape(I)[1]:
            print(
                "The number of pre-synaptic neurons must be the same as the number of columns of the input spike train"
            )
            return None
...
class STDP_synapse:
    ...
    def update_weights(self, spikes):
        ...
        if self.short_memory_trace:
            ...
            pre_trace += (
                self.pars.get("pre_trace_rest", 1) - pre_trace
            ) * pre_spk_vector
            post_trace += (
                self.pars.get("post_trace_rest", 1) - post_trace
            ) * post_spk_vector
        else:
        ...
        if self.constrain == "None":
            ...
            self.A_plus = (
                self.pars["A_plus"]
                * (np.absolute(self.w_max - self.W))
                ** self.pars.get("dynamic_weight_exponent", 1)
                * np.sign(self.w_max - self.W)
            )
            self.A_minus = (
                self.pars["A_minus"]
                * (np.absolute(self.W - self.w_min))
                ** self.pars.get("dynamic_weight_exponent", 1)
                * np.sign(self.W - self.w_min)
            )
    ...
    def plot_synapses(
        self, post_index=0, time_in_ms=False, subsampling=1, trace_index_list=None
    ):
        ...
        fig, ax = plt.subplots(
            4, figsize=(12, 14), gridspec_kw={"height_ratios": [1, 2, 2, 2]}
        )  # , sharex=True)

        # plot the weights as an image
        fig.colorbar(
            ax[0].imshow(W.T, cmap="viridis", aspect="auto"),
            ax=ax[0],
        ...
        ax[1].plot(time_steps[::subsampling], W[::subsampling, :], lw=1.0, alpha=0.7)
        ...
        ax[2].plot(time_steps, pre_trace, lw=1.0, alpha=0.05)
        df = pd.DataFrame(pre_trace[:, trace_index_list])
        df.plot(ax=ax[2], color=["r", "g", "b"], lw=1.0, alpha=1, legend=False)
        ...
        ax[3].plot(time_steps, post_trace, lw=1.0, alpha=0.7)
...
def Poisson_generator(
    dt, rate, N_pre, num_steps, myseed=False, herz_rate=False, batch_size=0
):
    ...
    if herz_rate:
        rate_per_bin = rate * (dt / 1000.0)
    else:
        rate_per_bin = rate

    # generate Poisson train
    poisson_train = 1.0 * (u_rand < rate_per_bin)

    return poisson_train


def random_shifted_trains(
    dt,
    num_steps,
    N_pre,
    ...
    if N_pre_correlated > N_pre:
        print(
            "The number of correlated neurons must be less than the total number of neurons"
        )
    ...
    for spk_t in spike_times[0]:
        random_shifts = np.random.randint(
            low=shift_values[0], high=shift_values[1], size=N_pre
        )
        within_the_range = lambda x: x > 0 and x < num_steps
        random_shifted_times = [
            spk_t + random_shifts[i]
            for i in range(N_pre_correlated)
            if within_the_range(spk_t + random_shifts[i])
        ]
        for time, index in zip(random_shifted_times, range(N_pre_correlated)):
            perturbed_spike_trains[time, index] = 1

    pre_spk_train = Poisson_generator(
        dt, rate=rate, N_pre=N_pre - N_pre_correlated, num_steps=num_steps
    )
...
def random_offsets(
    dt, num_steps, N_pre, rate=0.5, sections=10, sort_shifts=True, length_ratio=0.5
):
    ...
    shift_values = (
        np.random.randint(0, sections, N_pre)
        * (num_steps - original_length)
        // (sections - 1)
    )
    ...
    I_original = np.concatenate(
        [I, np.zeros((num_steps - original_length, N_pre))], axis=0
    )
