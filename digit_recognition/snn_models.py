...
class snn_mnist(nn.Module):
    ...
    def __init__(self, pars, input_size, n_neurons):
        ...
        if pars["store_records"]:
            self.t = pars[
                "store_subsampling_factor"
            ]  # this does not apply to the spikes
    ...
    def weight_initializer(self):  # sill in development
        ...
        if self.pars.get("weight_initialization_type", "clamp") == "clamp":
            ...
            W = (
                torch.rand_like(self.fc.weight.data) * (self.w_max - self.w_min)
                + self.w_min
            )
            ...
            raise NotImplementedError(
                "Only clamp, shift and norm_column weight initialization are implemented"
            )
    ...
    def get_records(self):
        ...
        mem_record = (
            torch.stack(self.neuron_records["mem"])
            .detach()
            .numpy()
        ...
        syn_record = (
            torch.stack(self.neuron_records["syn"])
            .detach()
            .numpy()
            .reshape(-1, self.n_neurons)
        )
        if self.pars["dynamic_threshold"]:
            threshold_record = (
                torch.stack(self.neuron_records["threshold"])
                .detach()
                .numpy()
                .reshape(-1, self.n_neurons)
            )
        ...
        pre_trace = (
            torch.stack(self.synapse_records["pre_trace"])
            .detach()
            .numpy()
            .reshape(-1, self.input_size)
        )
        post_trace = (
            torch.stack(self.synapse_records["post_trace"])
            .detach()
            .numpy()
            .reshape(-1, self.n_neurons)
        )
        W_record = (
            torch.stack(self.synapse_records["W"])
            .detach()
            .numpy()
            .reshape(-1, self.n_neurons, self.input_size)
        )
            ...
            for time_step in range(num_steps):
                ...
                if self.pars["refractory_period"] and time_step > 0:
                    ...
                    self.refractory_times = (
                        self.refractory_times + spk_prev * self.ref_time
                    )
                ...
                if self.pars["refractory_period"] and time_step > 0:
                    spk, syn, mem = self.compute_refractory_times(
                        spk, spk_prev, mem_prev, syn_prev, mem, syn, store=store
                    )
                ...
                post_syn_traces = self.update_post_synaptic_traces(
                    spk, post_syn_traces, store=store
                )
    ...
    def initialize_auxiliary_attributes(self, image):
        ...
        if self.pars["store_records"]:
            self.synapse_records["pre_trace"].append(
                pre_syn_traces[:: self.t].to(dtype=torch.float16)
            )
    ...
    def compute_refractory_times(
        self, spk, spk_prev, mem_prev, syn_prev, mem, syn, store
    ):
        """
    ...
    def update_post_synaptic_traces(self, spk, previous_post_syn_traces, store=True):
        ...
        if store:
            self.synapse_records["post_trace"].append(
                new_post_syn_traces.to(dtype=torch.float16)
            )
    ...
    def STDP_update(self, spk, pre_train, post_syn_traces, pre_syn_traces, store=True):
        ...
        if self.pars["STDP_type"] == None:
            ...
            if torch.sum(spk) == 0 and torch.sum(pre_train) == 0:
                ...
                LTD = (
                    torch.tensordot(post_syn_traces.T, pre_train, dims=1)
                    / self.batch_size
                )
            ...
            if spk.sum() == 0:
                ...
                dynamic_weights_constrain = (
                    self.w_max - self.fc.weight.data
                ) ** self.pars["mu_exponent"]
                dW = (
                    self.pars["learning_rate"]
                    * thresholded_pre_traces.mean(axis=0)
                    * dynamic_weights_constrain
                )
            ...
            if torch.sum(spk) == 0 and torch.sum(pre_train) == 0:
                ...
                W = (
                    W
                    + A_plus * LTP * (self.w_max - W) ** exponent
                    - A_minus * LTD * (W - self.w_min) ** exponent
                )
                # W = torch.clamp(W, min=self.w_min, max=self.w_max)
        else:
            raise NotImplementedError(
                "Only classic, offset and asymptotic STDP are implemented"
            )
    ...
    def forward_test(self, image):
        ...
        with torch.no_grad():
            ...
            for time_step in range(image.shape[0]):
                ...
                if self.pars["refractory_period"] and time_step > 0:
                    spk, syn, mem = self.compute_refractory_times(
                        spk, spk_prev, mem_prev, syn_prev, mem, syn, store=False
                    )
    ...
    def plot_simulation(
        ...
        neuron_index=0,
        n_highlight=10,
        plot_syn_conductance=False,
        plot_post_trace=False,
        ...
        ax[0].scatter(
            *torch.where(spk_rec),
            s=height / self.n_neurons,
            c="black",
            marker="|",
            lw=0.5,
        )  # scatter is incredible slow
        ax[0].set_ylim([0 - 0.5, self.n_neurons - 0.5])
        ax[0].set_title(
            f"Mean output rate: {spk_rec.mean():.5f} - Mean input rate: {tot_input_mean:.5f}, last 10000 time steps"
        )
        ...
        if threshold_rec is not None:
            ...
            ax[1].hlines(
                1, 1, self.tot_sim_time / self.t + 1, color="red", linestyle="--"
            )
        ...
        if plot_post_trace:
            ...
            ax[2 + plot_syn_conductance].set_title(
                f"Post-synaptic trace, mean: {post_trace_rec.mean():.2f}"
            )
        ...
        cor_weights.plot(
            ax=ax[2 + plot_syn_conductance + plot_post_trace],
            legend=False,
            color="tab:red",
            alpha=0.8,
            lw=2 * lw,
        ...
        if plot_weights_distribution:
            ...
            ax[3 + plot_syn_conductance + plot_post_trace].hist(
                W_rec[time_step, :n_highlight],
                bins=bins,
                color="tab:red",
                alpha=0.5,
                label=f"first {n_highlight} synapses",
            )
            ax[3 + plot_syn_conductance + plot_post_trace].hist(
                W_rec[time_step, n_highlight:],
                bins=bins,
                color="tab:blue",
                alpha=0.5,
                label=f"remaining {self.input_size - n_highlight} synapses",
            )
            ...
            ax[3 + plot_syn_conductance + plot_post_trace].set_title(
    ...
    def plot_simulation_interactive(self, manual_update=False):
        ...
        def main_plot(neuron_index=0, n_highlight=10):
            ...
            # plot the records
            fig, ax = plt.subplots(
                3, figsize=(12, 15), gridspec_kw={"height_ratios": [1, 1, 1]}
            )
            lw = max(min(100 / self.tot_sim_time, 1), 0.5)

            # plot the output spikes of the network
            tot_input_mean = self.num_input_spikes / (
                self.tot_sim_time * self.input_size
            )
            spk_rec = records["spk"].float()[-10000:, :]
            height = ax[0].bbox.height
            ax[0].scatter(
                *torch.where(spk_rec), s=2 * height, c="black", marker="|", lw=lw
            )
            ax[0].set_ylim([0 - 0.5, self.n_neurons - 0.5])
            ax[0].set_title(
                f"Mean output rate: {spk_rec.mean():.5f} - Mean inputrate: {tot_input_mean:.5f} last 10000 time steps"
            )
            ax[0].set_ylabel("Neuron index")
            ...
            uncor_weights = pd.DataFrame(W_rec[:, n_highlight:])
            cor_weights.plot(
                ax=ax[2], legend=False, color="tab:red", alpha=0.8, lw=2 * lw
            )
            uncor_weights.plot(
                ax=ax[2], legend=False, color="tab:blue", alpha=0.2, lw=lw
            )
...
def train_model(
    model, train_loader, val_loader=None, num_epochs=1, val_accuracy_record=True
):
    ...
    if val_accuracy_record and val_loader is None:
        raise ValueError(
            "If val_accuracy_record is set to True, val_loader must be provided"
        )
    ...
    for epochs in range(num_epochs):
        ...
        with tqdm(
            total=len(train_loader), unit="batch", ncols=120 + 40 * val_accuracy_record
        ) as pbar:
            ...
            if val_accuracy_record:
                ...
                temp_assignments = assign_neurons_to_classes(
                    model, val_loader, verbose=0
                )

                # compute the accuracy so far (anspnpi = average number of spikes per neuron per image)
                accuracy, anspnpi = classify_test_set(
                    model, val_loader, temp_assignments, verbose=0
                )
                ...
                pbar.set_postfix(
                    ...
                    ANSPNPI=f"{anspnpi:.2e}",
                )

                # store the records
                history["val_acc"].append(accuracy)
                history["anspnpi"].append(anspnpi)
                history["conf_assignments"].append(
                    np.sum(temp_assignments.conf_status + 0.0)
                )
            else:
...
def assign_neurons_to_classes(model, val_loader, verbose=1):
    ...
    for data_it, targets_it in val_loader:
        ...
        # forward pass
        model.eval()
        num_spk = model.forward_test(
            data_it
        )  # numpy array of shape (batch_size x n_neurons)
    ...
    conf_status = np.diff(sort_rate, n=1)[:, -1] > model.pars.get(
    ...
    df_assignments = pd.DataFrame(
        [assignments, conf_status], index=["assignments", "conf_status"]
    ).T

    if verbose > 0:
        df_rate = pd.DataFrame(
            spk_matrix / num_steps,
            index=[i for i in range(n_neurons)],
...
def classify_test_set(model, test_loader, df_assignments, verbose=1):
    ...
    for data_it, targets_it in test_loader:
        ...
        sorted_response = np.argsort(
            num_spk, axis=1
        )  # batch_size x n_neurons with indexes sorted accordingly to f
    ...
    averaged_num_spk_per_neuron_per_image = np.sum(
        model.anspnpi[-len(test_loader) :]
    ) / len(test_loader)
